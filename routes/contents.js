...
var BoardContents = require('../models/boardSchemaModel'); //db를 사용하기 위한 변수
var fs = require('fs');
...
router.get('/', function(req,res){
    ...
    BoardContents.count({deleted:false},function(err, totalCount){
       // db에서 최근 날짜 순으로 데이터들을 가져옴
        if(err) throw err;
        console.log("=========>게시판 글 개수 : "+totalCount);
        pageNum = Math.ceil(totalCount/limitSize);
        BoardContents.find({deleted:false}).sort({date:-1}).skip(skipSize).limit(limitSize).exec(function(err, pageContents) {
            if(err) throw err;
            res.render('board/board', {title: "Board", contents: pageContents, pagination: pageNum, searchWord: ''});
        });
    });
});
...
router.get('/search', function(req, res){
    ...
    BoardContents.count({deleted:false, $or:[{title:searchCondition},{contents:searchCondition},{writer:searchCondition}]},function(err, searchCount){
        ...
        BoardContents.find({deleted:false, $or:[{title:searchCondition},{contents:searchCondition},{writer:searchCondition}]}).sort({date:-1}).skip(skipSize).limit(limitSize).exec(function(err, searchContents){
            ...
            res.render('board/board', {title: "Board", contents: searchContents, pagination: pageNum, searchWord: search_word});
        });
    });
});
...
router.post('/reply', function(req, res){
    ...
    var reply_writer = req.user.email;
...
router.get('/reply', function(req, res) {
    ...
    BoardContents.findOne({_id: id}, {comments: {$slice: [skipSize, limitSize]}} , function(err, pageReply){
       ...
       console.log("pageReplypageReplypageReply!");
    });
});
...
router.get('/delete_reply', function(req, res) {
    ...
    var content_id = req.param('contentId');
    var index = 1;
    deleteComment(comment_id, content_id);

    res.redirect('/boards/view?mode='+index+'&id='+content_id);
});
...
function deleteComment(commentId,contentId) {
    ...
    var content_id = contentId;
        ...
        BoardContents.update({'_id':content_id}, {'$pull':{comments:{_id:comment_id}}},function(err){
            if(err) throw err;
        });
}
...
router.get('/view', function(req, res){
    ...
    var contentId = req.param('id');
    var index = req.param('mode');

    BoardContents.findOne({_id:contentId}, function(err, rawContent){
        if(err) throw err;

        if(!index==1){ //index가 1인 경우는 댓글 삭제 후 페이지 복귀시
        rawContent.count += 1;
        }    //조회수 증가, 
        ...
        rawContent.save(function(err){
            ...
            res.render('board/boardDetail',{title: "Board", content:rawContent, replyPage: reply_pg});
        });
    })
});
...
router.get('/newWrite', function(req, res){
    res.render('board/newWrite');
});

router.post('/boards/boardDetail', function(req, res){
     var contentId = req.param('id');

    BoardContents.findOne({_id:contentId}, function(err, rawContent){
        if(err) throw err;
        rawContent.count += 1;
        var reply_pg = Math.ceil(rawContent.comments.length/5); //올림계산하기(5/6 = 2)

        rawContent.save(function(err){
            if(err) throw err;

            res.render('board/boardModify',{title: "Board", content:rawContent, replyPage: reply_pg});
        });
    })
});

router.get('/boardModify', function(req, res){
   var contentId = req.param('id');
